...
// 11. [修正版] 样本验证数据导出 (Validation Data Export)
//     用于 2.1.2 节：人工标注(秋季) vs GLCZ全球产品(Demuzere et al.)
// ==============================================================

print('正在准备人工标注与 GLCZ (Demuzere) 的对比数据...');

// 1. 加载 GLCZ 全球产品 (100m 分辨率)
// ID: RUB/RUBCLIM/LCZ/global_lcz_map/latest
// Band 'LCZ': 1-17 (与你的体系一致)
var glczCollection = ee.ImageCollection("RUB/RUBCLIM/LCZ/global_lcz_map/latest");
var glczImg = glczCollection.mosaic().select('LCZ').rename('GLCZ_Label');

// 2. 准备样本数据 (以 2018 年秋季标注为基准)
// 为什么选秋季？因为你的论文逻辑中秋季是"年际修正"的基准，且植被特征明显
var validationSamples = seasonal_samples.map(function(f) {
  // 获取秋季标签 (属性名可能是 'autumn' 或 'AUTUMN'，这里做兼容)
  var au = f.get('autumn') || f.get('AUTUMN'); 
  // 你的标签是 1-17，GLCZ 也是 1-17，直接对齐
  return f.set('Manual_Label', ee.Number(au).toInt()); 
}).filter(ee.Filter.notNull(['Manual_Label'])); // 过滤掉未标注的点

// 3. 提取 GLCZ 在样本点的值
// 注意：GLCZ 是 100m 分辨率，scale 设置为 100 或 30 都可以
// 设置为 30 会以最近邻插值读取 100m 像素的值，这是合理的
var samplesWithGLCZ = glczImg.sampleRegions({
  collection: validationSamples,
  properties: ['Manual_Label'], // 保留你的人工标注
  scale: 10, // 采样分辨率
  geometries: false // 导出 CSV 不需要几何坐标
});

// 4. 导出 CSV
Export.table.toDrive({
  collection: samplesWithGLCZ,
  description: 'Validation_Manual_vs_GLCZ_Demuzere',
  fileFormat: 'CSV',
  folder: 'LCZ_Paper_Figs',
  selectors: ['Manual_Label', 'GLCZ_Label'] // 导出对比列
});