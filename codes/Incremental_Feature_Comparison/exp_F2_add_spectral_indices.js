...
// 原：print('L8 feature bands (30m, EPSG:3857):', l8Bands);
print('L8+指数 feature bands (30m, EPSG:3857):', l8Bands);


// ====================== 3) 人工200/类 + 高置信100/类：生成点位（简单版） ======================
var labelBand = 'Label';

// 3a) 人工标注面（直接使用原始面，不做几何清洗）
...
var SEED = 42;
var SHRINK_M = 20;  // ← 关键：轻度向内收缩距离（米）

var manualPts = ee.FeatureCollection(
  classes.map(function(c){
    c = ee.Number(c);
    // 取该类所有多边形并统一内缩
    var polys = manualFC_raw.filter(ee.Filter.eq('LCZ_label', c))
      .map(function(f){
        var g = f.geometry().buffer(-SHRINK_M);  // 向内收缩
        // 若收缩后为空则回退原几何
        var gFixed = ee.Geometry(ee.Algorithms.If(g.area(1).gt(0), g, f.geometry()));
        return ee.Feature(gFixed);
      });

    var geom = polys.geometry();
...
print('Manual points (≤200/class, with shrink):', manualPts.size());


// 3b) 高置信 GLCZ（100 m 分层）：每类最多 100 点
...
// ====================== 4) 在 30 m Landsat 特征上提取样本（同一投影系） ======================
...
// 原：print('Validation confusion matrix (L8, 30m)', cm);
print('Validation confusion matrix (L8+指数, 30m)', cm);

// 原：print('Validation OA (L8, 30m)', cm.accuracy());
print('Validation OA (L8+指数, 30m)', cm.accuracy());

// 原：print('Validation Kappa (L8, 30m)', cm.kappa());
print('Validation Kappa (L8+指数, 30m)', cm.kappa());

// 原：print('UA per class (L8, 30m)', cm.consumersAccuracy());
print('UA per class (L8+指数, 30m)', cm.consumersAccuracy());

// 原：print('PA per class (L8, 30m)', cm.producersAccuracy());
print('PA per class (L8+指数, 30m)', cm.producersAccuracy());
