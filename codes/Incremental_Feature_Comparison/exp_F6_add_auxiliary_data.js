...
/******************** 5) DEM / 夜光 / POP（全部对齐 30 m） ********/
// DEM（SRTM 30m）
var DEM = ee.Image('USGS/SRTMGL1_003').select('elevation')
  .clip(region).toFloat().unmask(0)
  .rename('DEM').reproject(proj3857_30).setDefaultProjection(proj3857_30);

// VIIRS 夜光（2018 年均值，VCMCFG）
var VIIRS = ee.ImageCollection('NOAA/VIIRS/DNB/MONTHLY_V1/VCMCFG')
  .filterDate(START, END).select('avg_rad')
  .mean().clip(region).toFloat().unmask(0)
  .rename('VIIRS').reproject(proj3857_30).setDefaultProjection(proj3857_30);

// WorldPop 人口（100m，取 2020 年以保证覆盖；如需严格 2018 可改年份）
var POP = ee.ImageCollection('WorldPop/GP/100m/pop')
  .filterDate('2020-01-01', '2021-01-01').select('population')
  .mean().clip(region).toFloat().unmask(0)
  .rename('POP').reproject(proj3857_30).setDefaultProjection(proj3857_30);

/******************** 6) 融合特征（全部 30 m） *****************/
var Feat_30m = Opt_feat_30m
  .addBands(S1_feats_30m)
  .addBands(DEM.updateMask(optMask))
  .addBands(VIIRS.updateMask(optMask))
  .addBands(POP.updateMask(optMask))
  .reproject(proj3857_30).setDefaultProjection(proj3857_30);

var inputBands = Feat_30m.bandNames();
print('All feature bands (L8+S2 + 指数 + L8纹理 + S1 + DEM/VIIRS/POP, 30m):', inputBands);

/******************** 7) 采样点：人工200/类(20m内缩) + 高置信100/类 ****/
var labelBand = 'Label';
...
print('Validation confusion matrix (All + DEM/VIIRS/POP, 30m)', cm);
...
var lcz2018_pred_30m = Feat_30m.classify(rf)
  .rename('LCZ_2018_pred_L8S2_L8Tex_S1_DEM_VIIRS_POP')
  .toInt16()
  .clip(region);
Map.addLayer(lcz2018_pred_30m, lczVis, 'LCZ 2018 预测 (L8+S2+指数+L8纹理+S1+DEM/VIIRS/POP, 30 m)', true);

/******************** 10) （可选）导出 *************************/
// Export.image.toAsset({
//   image: lcz2018_pred_30m,
//   description: 'LCZ_2018_L8S2_Idx_L8Tex_S1_DEM_VIIRS_POP_RF_30m',
//   assetId: 'users/你的用户名/LCZ_2018_L8S2_Idx_L8Tex_S1_DEM_VIIRS_POP_RF_30m',
//   region: region,
//   scale: 30,
//   maxPixels: 1e13
// });
